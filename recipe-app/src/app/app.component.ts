...
import {
  FridgeComponent,
  RecipeSearchParams,
} from "./components/fridge/fridge.component";
  ...
  template: `
    ...
    @if (isAuthenticated()) {
      <div class="min-h-screen bg-gray-100 font-sans flex flex-col">
        <header class="bg-white shadow-md sticky top-0 z-10 safe-top">
          <div
        ...
        <main class="flex-grow container mx-auto p-4 md:p-6">
          @switch (currentView()) {
            ...
            @case ("privacy") {
              <app-privacy-policy (goBack)="changeView('profile')" />
            }
          }
        </main>

        <nav
          class="sticky bottom-0 bg-white shadow-[0_-2px_10px_rgba(0,0,0,0.1)] safe-bottom"
        >
          <div class="container mx-auto flex justify-around">
            ...
            <button
              [class]="navButtonClass('fridge')"
              (click)="changeView('fridge')"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-6 w-6"
            ...
            <button
              [class]="navButtonClass('suggestions')"
              (click)="changeView('suggestions')"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-6 w-6"
        ...
        @if (planningRecipe(); as recipe) {
          ...
          >
            <div
              class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4 transform transition-all duration-300 scale-100"
            >
              <h3 class="text-xl font-semibold mb-2">
                {{ "modal_plan_recipe_title" | translate }}
              </h3>
              <p class="text-gray-600 mb-6">
                {{
                  "modal_plan_recipe_prompt"
                    | translate: { title: recipe.title }
                }}
              </p>
              <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                @for (day of daysOfWeek; track day) {
                  <button
                    (click)="assignRecipeToDay(day)"
                    class="p-3 bg-indigo-100 text-indigo-700 font-semibold rounded-lg hover:bg-indigo-200 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  >
                    {{ "day_" + day | translate }}
                  </button>
                }
        ...
        @if (isSelectingForDay(); as selection) {
          ...
          >
            <div
              class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg mx-4 transform transition-all duration-300 scale-100 flex flex-col"
            >
              <h3 class="text-xl font-semibold mb-2">
                {{
                  "modal_select_recipe_title"
                    | translate: { day: ("day_" + selection.day | translate) }
                }}
              </h3>

              @if (recipes().length > 0) {
                <p class="text-gray-600 mb-4">
                  {{ "modal_select_recipe_prompt" | translate }}
                </p>
                <div class="space-y-3 max-h-80 overflow-y-auto pr-2 -mr-2">
                  @for (recipe of recipes(); track recipe.title) {
                    <button
                      (click)="selectRecipeForDay(recipe)"
                      class="w-full text-left p-3 bg-gray-100 rounded-lg hover:bg-indigo-100 hover:text-indigo-700 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                ...
                <div class="text-center py-6">
                  <p class="text-gray-600">
                    {{ "modal_select_recipe_empty" | translate }}
                  </p>
                  <p class="text-sm text-gray-400 mt-1">
                    {{ "modal_select_recipe_empty_cta" | translate }}
                  </p>
                </div>
              }
              ...
              <div class="mt-6 text-right">
                <button
                  (click)="isSelectingForDay.set(null)"
                  class="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 font-medium"
                >
                  {{ "cancel" | translate }}
                </button>
        ...
        @if (cookingRecipe(); as recipe) {
          ...
          >
            <div
              class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-2xl mx-4 transform transition-all duration-300 scale-100 flex flex-col max-h-[90vh]"
            >
              ...
              <div class="overflow-y-auto pr-2 -mr-2 space-y-6">
                  ...
                  <div class="flex flex-wrap gap-2">
                    @for (tag of recipe.tags; track tag) {
                      <span
                        ...
                        class="px-3 py-1 text-sm font-semibold rounded-full flex items-center gap-1.5"
                      >
                ...
                <div
                  class="flex justify-around text-center text-sm text-gray-700"
                >
                  <div class="flex items-center space-x-2">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                ...
                @if (recipe.nutrition) {
                  ...
                  >
                    <h4 class="text-sm font-semibold text-orange-900 mb-3">
                      {{ "nutrition_per_serving" | translate }}
                    </h4>
                ...
                <div class="bg-indigo-50 p-4 rounded-lg">
                  ...
                  <div class="flex items-center gap-4">
                    <button
                      (click)="
                        adjustedServings.set(
                          Math.max(1, adjustedServings() - 1)
                        )
                      "
                    ...
                    <button
                      (click)="
                        adjustedServings.set(
                          Math.min(12, adjustedServings() + 1)
                        )
                      "
                      class="w-10 h-10 rounded-full bg-indigo-600 text-white font-bold hover:bg-indigo-700 transition-colors"
                    >
                    ...
                    <span
                      class="text-2xl font-bold text-indigo-600 min-w-[3rem] text-center"
                      >{{ adjustedServings() }}</span
                    >
                  </div>
                </div>
                ...
                <div>
                  <h4 class="text-lg font-semibold text-gray-700 mb-2">
                    {{ "suggestions_ingredients_title" | translate }}
                  </h4>
                  <ul
                    class="list-disc list-inside bg-gray-50 p-4 rounded-lg space-y-1 text-gray-600"
                  >
                    @for (
                      ingredient of getScaledIngredients(recipe);
                      track ingredient
                    ) {
                      <li>{{ ingredient }}</li>
                    }
                ...
                <div>
                  <h4 class="text-lg font-semibold text-gray-700 mb-2">
                    {{ "suggestions_instructions_title" | translate }}
                  </h4>
      ...
      <div
        class="min-h-screen bg-gray-100 font-sans flex items-center justify-center p-4"
      >
          ...
          <div class="bg-white rounded-2xl shadow-xl p-8 relative">
            ...
            <div class="space-y-4">
              <button
                ...
                class="w-full flex items-center justify-center py-3 px-4 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50 transition-colors"
              >
              ...
              <div class="relative my-6">
                ...
                <div class="relative flex justify-center text-sm">
                  <span class="px-2 bg-white text-gray-500">{{
                    "login_email_continue" | translate
                  }}</span>
                </div>
              </div>
...
export class AppComponent {
  ...
  isAuthenticated = computed(
    () => !!this.authService.currentUser() || this.guestMode(),
  );
  loginEmail = signal("");
  ...
  private setupStateSynchronization() {
    effect(() => {
      ...
      if (remoteUser) {
        if (
          ...
          JSON.stringify(remoteUser.mealPlan)
        ) {
    ...
    effect(() => {
      ...
      if (
        remoteUser &&
        JSON.stringify(localPlan) !== JSON.stringify(remoteUser.mealPlan)
      ) {
    ...
    effect(() => {
      ...
      if (
        remoteUser &&
        JSON.stringify(localList) !== JSON.stringify(remoteUser.shoppingList)
      ) {
        this.firestoreService.updateUser(remoteUser.uid, {
  ...
  navButtonClass(view: View): string {
    const baseClasses =
      "flex flex-col items-center justify-center p-3 text-gray-500 transition-colors w-1/5 hover:bg-gray-100 focus:outline-none";
  ...
  private async awardPoints(points: number, statType: string, amount: number) {
    if (this.guestMode()) {
      ...
      this.notificationService.showPointsGained(
        points,
        reasonMap[statType] || "Action completed",
      );
    ...
    const updatedAchievements = {
      ...
      recipesCooked:
        statType === "recipes_cooked"
          ? achievements.recipesCooked + amount
          : achievements.recipesCooked,
      recipesGenerated:
        statType === "recipes_generated"
          ? achievements.recipesGenerated + amount
          : achievements.recipesGenerated,
      shoppingItemsAdded:
        statType === "shopping_items"
          ? achievements.shoppingItemsAdded + amount
          : achievements.shoppingItemsAdded,
      mealPlansCreated:
        statType === "meal_plans"
          ? achievements.mealPlansCreated + amount
          : achievements.mealPlansCreated,
      portionsAdjusted:
        statType === "portions_adjusted"
          ? achievements.portionsAdjusted + amount
          : achievements.portionsAdjusted,
      languagesUsed: [
        ...new Set([
          ...achievements.languagesUsed,
    ...
    const newAchievements =
      this.gamificationService.checkAchievements(updatedAchievements);
    ...
    const reasonMap: { [key: string]: string } = {
      ...
      recipes_generated: this.translationService.translate(
        "notif_recipe_generated",
      ),
    ...
    if (streakResult.streakBonus > 0) {
      this.notificationService.showStreakBonus(
        streakResult.newStreak,
  ...
  private parseIngredientOLD(text: string): {
    ...
    if (unit) {
      ...
      const isProtein = proteinKeywords.some((keyword) =>
      ...
      const isVolumeUnit = volumeUnits.some(
        (v) => v.toLowerCase() === unitLower,
      );
      const isWeightUnit = weightUnits.some(
        (w) => w.toLowerCase() === unitLower,
      );
